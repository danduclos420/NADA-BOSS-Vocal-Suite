...
const getFilterResponse = (x, band) => {
  ...
  const f = x / 400; // Normalized frequency 0-1
  const targetF = freq / 400;
  const g = (gain - 0.5) * 100; // -50 to +50
  
  if (type === 'cut-low') {
    return f < targetF ? -100 * Math.pow(1 - f/targetF, 2) : 0;
  }
  if (type === 'cut-high') {
    return f > targetF ? -100 * Math.pow((f - targetF)/(1 - targetF), 2) : 0;
  }
  // Bell curve
...
const Knob = ({ label, initialValue = 0.5, size = 40, showValueInside = false, color = "var(--neon-red)" }) => {
...
const ProQ3View = () => {
  ...
  return (
    <div style={{ width:'100%', height:'180px', background:'#08090b', border:'1px solid #222', borderRadius:4, position:'relative', overflow:'hidden', boxShadow:'inset 0 0 40px #000' }}>
       ...
       <div style={{ position:'absolute', top:0, left:0, width:'100%', height:'100%', opacity:0.05, backgroundImage:'linear-gradient(#111 1px, transparent 1px), linear-gradient(90deg, #111 1px, transparent 1px)', backgroundSize:'20px 20px' }}></div>
       <svg viewBox="0 0 400 180" style={{ position:'absolute', top:0, left:0, width:'100%', height:'100%' }}>
          ...
          {bands.map((b, i) => (
             <g key={i}>
                ...
                <circle cx={b.freq} cy={100 - (b.gain-0.5)*100} r="3" fill={b.color} style={{ cursor:'pointer' }} />
             </g>
          ))}
       </svg>
       <div style={{ position:'absolute', bottom:5, width:'100%', display:'flex', justifyContent:'center', gap:5 }}>
          {bands.map((b,i) => <div key={i} style={{ fontSize:6, color:'#444', border:'1px solid #1a1c22', padding:'1px 3px', background:'#000' }}>B{i+1}</div>)}
       </div>
    </div>
  );
};
...
const PAZView = () => (
  <div style={{ width:'100%', height:'180px', background:'#08090b', border:'1px solid #222', borderRadius:4, position:'relative', display:'flex', flexDirection:'column', padding:10, boxSizing:'border-box', boxShadow:'inset 0 0 40px #000' }}>
     ...
     <div style={{ flex:1.2, display:'flex', gap:2, alignItems:'flex-end', borderBottom:'1px solid #111', paddingBottom:10 }}>
        {Array.from({length:32}).map((_,i) => <div key={i} style={{ flex:1, height: `${10 + Math.random()*70}%`, background: i > 25 ? 'var(--neon-red)' : '#111' }}></div>)}
     </div>
