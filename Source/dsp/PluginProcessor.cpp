...
void NADAAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    bool aiEnabled = apvts.getRawParameterValue("AI_ENABLED")->load() > 0.5f;

    // If AI is enabled, potentially override some settings
    if (aiEnabled && aiState.aiEnabled) {
        // AI parameters override user params (optional)
    }
...
void NADAAudioProcessor::updateDSPChain()
{
    ...
    *pultec.high.coefficients = *juce::dsp::IIR::Coefficients<float>::makeHighShelf(mSampleRate, 10000.0f, 0.7f, juce::Decibels::decibelsToGain(pHighBoost));
    ...
    for (int i = 0; i < 4; ++i) {
        float gain = 1.0f + sslDrive * 0.5f;
        *ssl.bands[i].coefficients = *juce::dsp::IIR::Coefficients<float>::makePeakFilter(mSampleRate, freqs[i], 1.0f, gain);
    }
...
{
    aiState.statusText = "ANALYZING SPECTRAL DATA...";
    auto profile = spectralAnalyzer.analyzeBuffer(analysisBuffer);
    aiState.lastProfile = profile;
    auto mixParams = aiMixer.generateMixingParameters(profile);
    aiState.lastMixParams = mixParams;
    applyAIMixingParameters(mixParams);
    aiState.statusText = "VOCAL OPTIMIZED - RADIO READY ✓";
    aiState.isAnalyzing = false;
}

void NADAAudioProcessor::applyAIMixingParameters(const AIMixer::MixingParameters& params)
{
    for (int i = 0; i < 6; ++i) {
        if (auto* freqParam = apvts.getParameter("EQ_BAND_" + juce::String(i+1) + "_FREQ")) {
            freqParam->setValueNotifyingHost((params.eqBands[i].frequency - 20.0f) / (20000.0f - 20.0f));
        }
        if (auto* gainParam = apvts.getParameter("EQ_BAND_" + juce::String(i+1) + "_GAIN")) {
            gainParam->setValueNotifyingHost((params.eqBands[i].gain + 18.0f) / 36.0f);
        }
        if (auto* qParam = apvts.getParameter("EQ_BAND_" + juce::String(i+1) + "_Q")) {
            qParam->setValueNotifyingHost((params.eqBands[i].q - 0.1f) / 9.9f);
        }
    }
    if (auto* thrParam = apvts.getParameter("FET_THRESH")) {
        thrParam->setValueNotifyingHost((params.fet1176Threshold + 60.0f) / 60.0f);
    }
    if (auto* ratParam = apvts.getParameter("FET_RATIO")) {
        ratParam->setValueNotifyingHost((params.fet1176Ratio - 4.0f) / 16.0f);
    }
}
